...
var ExcellentExport = (function() {
    ...
    var template = { excel: '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40"><head><meta name=ProgId content=Excel.Sheet> <meta name=Generator content="Microsoft Excel 11"><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>{worksheet}</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--></head><body><table>{tableb}{table}</table></body></html>' };
    var csvDelimiter = ",";
    var csvNewLine = "\r\n";
    ...
    var ee = {
        ...
        csv: function(anchor, table, tableb, delimiter, newLine) {
            ...
            table = get(table);
            tableb = get(tableb);
            var csvData = "\uFEFF" + tableToCSV(tableb) + tableToCSV(table);
